...
class ProductController {
    async productHandler(req: Request, res: Response) {
        try {
            ...
            if (!url) {
                return res.status(422).json({ error: {message: 'É preciso informar a URL.' }});
            }

            // Verifica se o dominio é permitido
            if (!productService.isDomainAllowed(url)) {
                return res.status(422).json({ error: {message: 'Não é permitido consultar informações no dominio informado.' }});
            }
